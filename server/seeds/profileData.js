// Side-effect-free demo profile data — already in frontend shape
// (camelCase keys, arrays). The CLI seed script
// (`server/db-seed-profile.js`) re-imports this constant and writes it
// to SQLite via `saveProfile`. Feature 020's portfolio demo seed
// (`src/data/demoSeed.js`) imports the same constant verbatim, and the
// parity test in `tests/data/demoStore.test.js` checks equality. Keep
// this file free of any `./db.js` / `initSchema()` / `saveProfile()` /
// `process.exit()` side effects so the import is safe from any context.
//
// Parity rule: when this object changes, update the demo fixture in
// `src/data/demoSeed.js` in the same PR.
