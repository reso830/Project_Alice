...
### Decision
Implement the Terms & Conditions and Privacy Policy as a globally importable component `src/components/LegalModal.js` that dynamically builds and mounts the modal dialog (or bottom sheet on mobile) onto `document.body` when triggered.
...
- To ensure ease of integration and keep state management clean:
  - The modal will mount/unmount directly on `document.body`.
...
### Rationale
- Swapping the raster brand icon `Alice_White.png` for the color vector `Alice-Sigil-color.svg` at `64x64px` improves sharpness and legibility across high-density retina displays.
...
- The License section replaces stub indicators with interactive triggers that call `LegalModal.open('terms')` / `LegalModal.open('privacy')`.
