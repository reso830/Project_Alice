...
Follow-up finding from this re-review (now resolved, see below): item 1 was reopened a second time because `docs/features/2.0.0-smart-intake-ai-assistance/044-hosted-startup-performance.md`'s "Hosted-only scoping" bullet still said local/portable were handled by "local/portable's own Express static-serving route" — true for portable, false for the documented local source-checkout workflow (`npm run dev`), where Vite serves `index.html` directly.
