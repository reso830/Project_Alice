...
- [x] T033 Walk US0–US8 on a hosted deployment
  ...
  - **Implementation note (2026-07-07)**: walked against the live PR #116 Vercel preview (deployment protection required staying logged into Vercel in a normal browser tab; DevTools "Disable cache" substituted for incognito). Results:
    - **US1, US2, US7, US8** — confirmed directly on the preview (throttled Slow 4G): branded loader visible before content; Welcome reached fast and unaffected by a blocked `/api/health` (proving the no-health-wait routing); `Calendar-*.js`/`Profile-*.js` confirmed absent from initial load and fetched only on navigation; zero `fonts.googleapis.com` requests, self-hosted `.woff2` served.
    - **US3** — confirmed on the preview: Tracker-boot skeleton visible before real application rows.
    - **US6** — confirmed via DevTools Rendering panel (`prefers-reduced-motion: reduce` emulation).
    - **US5** — **not** exercised against the shared preview (deliberately misconfiguring a shared deployment isn't appropriate); reproduced the exact server/frontend mismatch locally instead (`APP_RUNTIME=hosted` + fake Supabase server vars + `SKIP_HOSTED_SCHEMA_CHECK=true` for the backend, no `VITE_SUPABASE_*` for the frontend) — ConfigError rendered immediately, no Welcome/shell flash.
    - **US4** — attempted on the preview via DevTools throttling (10 kbps, high latency, 3G presets); could not force a genuine hang (throttling fails/succeeds fast rather than truly stalling a request indefinitely). **Waived** per explicit decision — this path is already covered deterministically by `tests/main.test.js`'s fake-timer test (a permanently-pending health promise), which browser throttling can't reliably reproduce anyway.
    - **Local dev + portable**: exceeded the "spot-check" bar — ran `npm run dev`/`npm run server:dev` directly (confirmed no regressions), and separately built + extracted + launched the real `alice-v1.12.0-win-x64.zip` portable package, confirming via direct HTTP requests against the running server that the loader markup and Google Fonts references are absent, lazy chunks and self-hosted fonts serve correctly (`200`), and the API responds.
    - Two pre-existing, unrelated issues surfaced during this walkthrough and filed separately (not part of 044's scope): [#117](https://github.com/reso830/Project_Alice/issues/117) (duplicate password-reveal eye icon, Edge native + custom toggle overlap) and [#118](https://github.com/reso830/Project_Alice/issues/118) (ConfigError's white sigil logo low-contrast on the warm cream background).

- [x] T034 Confirm the perf gate against baseline
  - **Target**: [metrics.md](metrics.md)
  ...
  - **Implementation note (2026-07-07)**: real hosted FCP/LCP numbers remain unavailable pre-merge — Vercel Speed Insights only records field data from the **production** domain, not preview deployments, so there is no way to obtain them before this PR ships. Formally recorded as **ACCEPTED** in [metrics.md](metrics.md)'s Residual Risk section: the structurally-verified facts already in this file (bundle-size reduction, removed render-blocking requests, loader hosted-only delivery) plus the US1–US8 manual walkthrough above are accepted as sufficient for merge; the perf gate itself will be confirmed post-deploy once production Speed Insights has data.
