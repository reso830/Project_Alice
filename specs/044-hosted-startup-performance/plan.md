...
- **New Dependencies**: WS5 adds `@fontsource/sora` (^5.2.8) and `@fontsource/dm-mono` (^5.2.7) — self-hosting was chosen over `<link rel=preload>` (see research.md D7). Justification: removes the render-blocking `fonts.googleapis.com`/`fonts.gstatic.com` third-party requests from the critical path entirely (not just made non-blocking), and is more consistent with the project's local-first principle (no third-party network dependency for fonts in local/portable/offline builds). Both packages ship pre-built `@font-face` CSS + `.woff2`/`.woff` files per weight; imported via `src/styles/main.css` (`@import '@fontsource/sora/400.css'` etc. for weights 400/500/600/700, `@fontsource/dm-mono/{400,500}.css`), which Vite bundles into the same CSS output already loaded asynchronously relative to first paint — no new render-blocking resource is introduced, and the inlined startup loader (unaffected either way, since it's fully self-contained) still paints independent of font availability. No other new dependencies.
