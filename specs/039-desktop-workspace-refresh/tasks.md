...
### T001 `[ ]` — Create the `OPanel` collapsible panel shell
...
### T003 `[ ]` [P] — Create the `ClampText` line-clamp primitive
...
### T004 `[ ]` [P] — Unit tests for `ClampText`
...
### T005 `[ ]` — Add `embedded` option to `CompatibilityModule.render`
...
### T006 `[ ]` [P] — Add `bare` (headerless) option to `Timeline.render`
...
### T007 `[ ]` — Rewrite `Modal._renderBody()` to the 5-panel `.pbody` stack
...
### T009 `[ ]` — Update/extend Modal body tests for the panelized structure
...
### T010 `[ ]` [P] — Regression tests: required fields, URL, status/date in the panelized body
- **Target**: `tests/components/Modal.test.js`
...
### T011 `[ ]` — Add `variant`/`target`/`onClosed` + `requestClose()` to `Modal`
- **Target**: `src/components/Modal.js`
...
### T013 `[ ]` — Tests for the pane variant
- **Target**: `tests/components/Modal.test.js`
...
### T014 `[ ]` — Master-detail wrapper + responsive detection in `Tracker`
...
### T016 `[ ]` [P] — Full-width pagination below the split
- **Target**: `src/pages/Tracker.js`
...
### T017 `[ ]` [P] — Master-detail layout styles
- **Target**: `src/styles/main.css`
...
### T018 `[ ]` — Tracker layout + empty-pane tests
...
### T019 `[ ]` — `selected` option on `Card.render`
...
### T020 `[ ]` [P] — Card selected-state test
...
### T021 `[ ]` — Selection state + pane routing in `Tracker`
- **Target**: `src/pages/Tracker.js`
...
### T024 `[ ]` — Dirty-switch guard in `selectApplication`
- **Target**: `src/pages/Tracker.js`
...
### T025 `[ ]` — Selection persistence across filter/sort/page/view
- **Target**: `src/pages/Tracker.js`
...
### T026 `[ ]` [P] — Breakpoint handoff (modal ↔ pane at 1100px)
- **Target**: `src/pages/Tracker.js`
...
### T027 `[ ]` — Tests: dirty-switch, persistence, handoff
- **Target**: `tests/pages/Tracker.test.js`
...
### T028 `[ ]` — Archived panels render read-only in the pane
- **Target**: `src/components/Modal.js`
...
### T029 `[ ]` — Archived + tablet/mobile regression tests
...
### T034 `[ ]` — Correct design docs to the as-built panel order
...
### T036 `[ ]` — Browser smoke walk of all user stories
