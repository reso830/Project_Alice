...
- [ ] T006 [P] Implement `getSalaryBounds(apps)` in `src/utils/filterSort.js` — scans all apps using `parseSalaryRange`, finds overall min (lower bounds) and max (upper bounds), rounds each to nearest `$1k`; returns `{ min: 0, max: 200_000 }` when no parseable salary data exists
...
- [ ] T019 [P] Add salary parser tests to `tests/utils/filterSort.test.js` — `parseSalaryLower`: `"$110k-$130k"` → 110000, `"$95,000-$115,000"` → 95000, `""` → null, `"competitive"` → null; `parseSalaryRange`: valid string → `{min, max}`, single-value `"$120k"` → `{min: 120000, max: 120000}`, `"$120k+"` → `{min: 120000, max: 120000}`; `getSalaryBounds`: verify rounding and fallback
...
- [ ] T041 [US1] Update `unmount()` in `src/pages/Tracker.js` — reset `_filterState = { ...DEFAULT_FILTER_STATE }`, `_sortState = { ...DEFAULT_SORT_STATE }`, `_salaryBounds = { min: 0, max: 200_000 }`, `_toolbarEl = null`
