...
- **WS1 — `authStore` foundation.** Recovery-URL detection, the allow-list-only-`PASSWORD_RECOVERY` guard (research.md D1), the two new status values (`password-recovery`, `recovery-expired`), and the `validatePassword` consolidation into `src/utils/validate.js`. Nothing user-visible ships yet — this is the shared groundwork WS3/WS4 depend on. *The event-ordering claim is now source-verified (research.md D1); the live end-to-end path (real email/redirect/browser) still needs a real recovery link before building WS4 on top of it.*
...
### Risk: a recovery link's early event(s) can precede `PASSWORD_RECOVERY`

**Corrected 2026-07-10 (Phase 01 implementation) — see research.md D1 for the full account.** The original hypothesis here (opening a recovery link fires `SIGNED_IN` before `PASSWORD_RECOVERY`) was based on a web search and has since been checked against the actual installed/pinned `@supabase/auth-js@2.105.4` source. The real sequence is `INITIAL_SESSION` (auth-js's own replay to every newly-registered `onAuthStateChange` callback, carrying the just-saved recovery session) then `PASSWORD_RECOVERY` — not `SIGNED_IN`. The risk and mitigation below are otherwise unchanged: an early event carrying a real session, left unguarded, would briefly resolve `authStore` to `status: 'authenticated'`, and `main.js`'s `render()` could mount the real authenticated app shell for a frame before `PASSWORD_RECOVERY` corrects it — the same class of "flash before the correct destination mounts" risk 044 solved for `local-mode`/ConfigError.

**Mitigation**: read `location.hash`/`location.search` synchronously (before any async Supabase call) for Supabase's recovery marker (`type=recovery`). If present, `authStore.init()` sets a `pendingRecovery` flag *before* calling `getSession()`/registering `onAuthStateChange`, and while that flag is set, **any event that isn't literally `PASSWORD_RECOVERY`** is held, not applied as `authenticated` — the store waits specifically for `PASSWORD_RECOVERY` (which is now guaranteed relevant, not speculative) before resolving to a new `password-recovery` status. Built this way (an allow-list of one event, not a block-list of `SIGNED_IN` specifically), the guard turned out to be correct regardless of which other event actually arrives first — no code change was needed when the event-ordering hypothesis was corrected, only the comments and tests. A short timeout (mirroring 044's boot-timeout pattern, ~5–10s) covers the case where neither event ever arrives (a genuinely dead/malformed link) by resolving to `recovery-expired` instead. This also directly implements the expired-link acceptance criterion (AC-7) — the same mechanism serves both "avoid flashing the app shell" and "detect an invalid link on load," so this is not two problems, it's one.
...
- `src/data/authStore.js` — recovery-URL detection, reordered `onAuthStateChange` registration, allow-list-only-`PASSWORD_RECOVERY` guard (research.md D1), two new status values, `signOut()` reuse for post-reset.
...
- **Recovery-link event ordering** (the primary architectural risk — see Architecture above and research.md D1). Mitigation: synchronous URL-marker check + an allow-list guard (only `PASSWORD_RECOVERY` resolves it; every other event is held) + timeout fallback to `recovery-expired`. **Update (Phase 01, 2026-07-10)**: the event-ordering claim was checked against the actual installed `@supabase/auth-js@2.105.4` source rather than left as a documentation-only assumption — the real sequence is `INITIAL_SESSION` then `PASSWORD_RECOVERY` (research.md D1), and the guard's allow-list design meant no code change was needed once this was discovered. The one remaining gap is a live end-to-end test (real email, real redirect, real browser) — not yet run.
...
- **Manual/exploratory**: a real recovery-link click against a test Supabase project, to confirm the live end-to-end path (real email, redirect, browser) — the event-ordering logic itself is now source-verified (research.md D1), but the full live round-trip is the one thing that can't be verified by reading code or by unit tests against a mocked client.
