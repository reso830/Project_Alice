...
**Invariants**
- A bare `SIGNED_IN` event that arrives while a recovery-URL guard is pending does **not** resolve to `authenticated` — it is held until either `PASSWORD_RECOVERY` confirms it (→ `password-recovery`) or the guard times out (→ `recovery-expired`). See plan.md Architecture / research.md D1.
