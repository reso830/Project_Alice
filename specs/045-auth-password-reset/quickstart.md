...
- **WS1 (authStore foundation)**: unit-test the recovery-URL guard directly (simulate a `type=recovery` URL + a `SIGNED_IN` then `PASSWORD_RECOVERY` event pair; confirm `authStore` never transiently resolves `authenticated`). Then, **once**, do a real recovery-link click against a test Supabase project and confirm the assumed event ordering (research.md D1) — this de-risks WS4 before it's built.
