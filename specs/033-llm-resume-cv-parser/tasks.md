...
- [ ] T002 [P] Unit tests for the AI settings store (`tests/data/aiSettings.test.js`)
...
- [ ] T003 [P] Implement the AI settings store (`src/data/aiSettings.js`)
...
- [ ] T004 [P] Unit tests for the LLM parser service (`tests/services/llmParser.test.js`)
...
- [ ] T006 [P] Server tests for `POST /api/resume/extract` (`tests/server/resumeExtract.test.js`)
...
- [ ] T007 Add `POST /api/resume/extract` endpoint (`server/routes/resume.js`)
...
- [ ] T008 [P] Server tests for `/api/resume/parse` JSON text mode (`tests/server/resume.text.test.js`)
...
- [ ] T010 [P] Extend the resume API client (`src/services/resumeApi.js`)
...
- [ ] T013 [US1] Thread the AI-field set through Edit Profile merge (`src/pages/ProfileEdit.js`)
...
- [ ] T014 [US2] Tests for the Profile AI settings section (`tests/pages/profile.aiSettings.test.js`)
...
- [ ] T015 [US2] Add the "AI Resume Parsing" settings section (`src/pages/Profile.js`)
...
- [ ] T017 [US2] Add the consent gate to ResumeImport (`src/components/ResumeImport.js`)
...
- [ ] T018 [US3] Tests for fallback + truncation behavior (`tests/components/ResumeImport.test.js`)
...
- [ ] T019 [US3] Implement fallback + truncation notice in ResumeImport (`src/components/ResumeImport.js`)
...
- [ ] T020 [US4] Tests for AI indicators (`tests/pages/profileEdit.aiIndicators.test.js`)
...
- [ ] T021 [US4] Render + clear AI indicators (`src/pages/ProfileEdit.js`)
...
- [ ] T030 `README.md` — Features bullet(s) for AI resume parsing (BYOK, browser-only key, opt-in); update `Current version` to `1.3.0`. No per-feature `specs/…` link under Further Reading.
- [ ] T031 `docs/deployment.md` — short BYOK note: AI parsing is browser-side and adds **no server env vars / no runtime-mode change**; link `quickstart.md`. (No env-var table change.)
- [ ] T032 `docs/REPO_MAP.md` — add entries for new files (`src/data/aiSettings.js`, `src/services/llmParser.js`, new tests) and the `/api/resume/extract` route; update `server/routes/resume.js` and `src/services/resumeApi.js` descriptions; add/confirm the Spec Packages row for `specs/033-llm-resume-cv-parser/`.
