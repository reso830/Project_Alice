...
- **Result (2026-06-16)**: implemented as a **flat per-band label color** — white on the High/Great fills, dark `#111827` on Low/Medium/track — via `getLabelColor`. The `text-shadow` halo approach was **tried and rejected by the user** as *less* readable. **Accepted tradeoff**: white-on-green is below WCAG AA (High `#16A34A` ≈ 3.3:1, Great `#22C55E` ≈ 2.3:1) but is the most readable option on the actual render per the user; residual risk noted (lower-vision / poor-display users). **Decision under consideration (optional follow-up):** recolor the **Great** fill to `#2563EB` (white ≈ 5.2:1, AA-pass) and optionally darken **High** to `#15803D` (≈ 4.6:1) to close the AA gap while keeping white text — to apply if it looks right.
