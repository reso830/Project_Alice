...
- **FR-005**: The **skills** category MUST weight each matched required skill by the profile's 1–5 proficiency for that skill (a stronger skill contributes more than a weaker one). Matched JD `preferredSkills` MUST contribute **partial credit** (a smaller, capped bonus, also proficiency-weighted) that supplements but cannot outweigh required-skill coverage — a profile missing required skills cannot reach a high skills sub-score on preferred matches alone.
...
- **Default weighting (initial proposal, configurable)**: skills weighted highest, then role alignment, experience, certifications, keywords; absent categories drop out and the rest renormalize to sum to the full scale. Exact numbers to be finalized in `plan.md`.
