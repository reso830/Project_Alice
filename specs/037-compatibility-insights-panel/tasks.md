...
### T001 `[ ]` — Column mappings for `compat_analysis` and `compat_scored_at`
...
### T002 `[ ]` [P] — Remove `compatNotes` from writable schema; add new fields to non-writable block
...
### T004 `[ ]` [P] — Supabase migration + `assertHostedSchema` probes
...
### T005 `[ ]` — App model: `normalizeApplication` for new fields
...
### T006 `[ ]` — Extend `recomputeActive` to stamp `compatScoredAt`
...
### T007 `[ ]` — Stamp `compatScoredAt` in application create/update route
...
### T008 `[ ]` — Extract `llmClient.js` from `llmParser.js`
...
### T009 `[ ]` — `compatNotesService.js` — prompt building + LLM call
...
### T010 `[ ]` — `api.saveCompatNotes()` in frontend API service
...
### T011 `[ ]` — Server route: `POST /api/applications/:id/compat-notes`
- **Target**: `server/routes/applications.js`
...
### T012 `[ ]` [P] — `src/utils/skillProficiency.js`
...
### T013 `[ ]` [US1, US2] — Score ring SVG and compatibility tier helpers
...
### T015 `[ ]` [US2] — Expanded panel + expand/collapse toggle
- **Target**: `src/components/CompatibilityModule.js`
...
### T016 `[ ]` [US3] — Notes region: `none` and `generating` states
- **Target**: `src/components/CompatibilityModule.js`
...
### T017 `[ ]` [US3, US2] — Notes region: `fresh` state + Show more/less
- **Target**: `src/components/CompatibilityModule.js`
...
### T018 `[ ]` [US4] — Notes region: `stale` state
- **Target**: `src/components/CompatibilityModule.js`
...
### T019 `[ ]` [US5] — Notes region: `error` state
- **Target**: `src/components/CompatibilityModule.js`
...
### T020 `[ ]` [US6] — `no-profile` availability state
- **Target**: `src/components/CompatibilityModule.js`
...
### T021 `[ ]` [US3, US4, US5] — Generation orchestration: wiring the LLM call
- **Target**: `src/components/CompatibilityModule.js`
...
### T022 `[ ]` [US1, US2] — Responsive layout (mobile bottom sheet)
...
### T023 `[ ]` — Add `profile` param to `Modal.open()`; Tracker.js profile fetch
...
### T024 `[ ]` — Upgrade skill chip editors to proficiency-coded chips (row 6)
...
### T025 `[ ]` — Replace `createCompatField` + `compatNotes` with `CompatibilityModule`; fix field order
- **Target**: `src/components/Modal.js`
...
### T026 `[ ]` — `demoStore.saveCompatNotes` + `compatScoredAt` stamping
...
### T027 `[ ]` [P] — Seed `compat_analysis` and `compat_scored_at` in demo data
...
### T028 `[ ]` [P] — Server column wiring integration test
...
### T029 `[ ]` [P] — `CompatibilityModule` full state machine coverage
...
### T038 `[ ]` [US1] — Score + skills at a glance
...
### T039 `[ ]` [US2] — Expand for full context
