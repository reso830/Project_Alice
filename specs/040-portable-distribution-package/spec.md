...
- Q: (Amendment, 2026-06-22) Should launching the package twice start a second instance? → A: **No — single instance.** Before binding, the launcher probes the configured port's `/api/health`; if an Alice instance is already serving there, it **opens the browser to the existing instance and exits without starting a second server** (FR-033). This avoids a second window on a different port + a separate `localStorage` origin (where the AI key wouldn't be present) and a second SQLite connection to the same `data/alice.db`. If the configured port is held by a **non-Alice** process, the launcher still falls back to the next free local port (FR-032) — the two behaviors are complementary.
...
**Independent Test**: Induce each failure in turn — (a) remove/rename a required application file, (b) make the bundled runtime unlaunchable, (c) occupy the default port with a **non-Alice** process, and (d) launch a **second** time while Alice is already running — then launch. Confirm (a) and (b) produce a clear, persistent error message (no silent vanish), (c) auto-recovers onto the next free port and opens the browser there, and (d) focuses the existing instance (opens the browser to it) without starting a second server.
...
3. **Given** the default port is held by a **non-Alice** process, **When** the user launches, **Then** Alice automatically selects the next available local port, opens the browser to that port, and indicates the port in use — rather than failing.
4. **Given** an Alice instance is already running on the configured port, **When** the user launches again, **Then** the launcher opens the browser to the existing instance and exits without starting a second server (single instance).
...
- **Alice already running / double launch**: a second launch detects the running instance via a `/api/health` probe on the configured port and simply **opens the browser to it, then exits** — no second server, no second window on a different port, no second SQLite connection (FR-033). (If the configured port is instead held by a non-Alice process, the launcher falls back to the next free local port per FR-032.)
- **Single-instance limitations (accepted in 040; robust fix deferred to 041)**: the FR-033 probe keys on *port + health response only*, so it (1) cannot tell two **separate** installs apart if they share the default port — the second launch would focus the first install's instance — and (2) does not detect the same install when it is running on a *fallback* port. Mitigation in 040 is documentation: give each concurrent install a distinct `config/settings.json` `port`. The robust per-install guard (lock file + clean restart handshake, forward-compatible across versions) is **owned by 041**, where exactly-one-instance during stop → swap → restart becomes load-bearing for safe updates and migrations.
...
- **FR-032**: When the default port is held by a **non-Alice** process, the launcher MUST automatically select the next available **local** port and open the browser to it, rather than failing; the auto-selected port MUST remain bound to localhost only.
- **FR-033**: The package MUST run as a **single instance**. Before binding, the launcher MUST probe the configured port's `/api/health`; if an Alice instance is already serving there, it MUST NOT start a second server — it MUST open the browser to the existing instance and exit. A non-Alice process on the port, a refused connection, or a timeout MUST be treated as "not running" so the launcher proceeds to the normal port-fallback path (FR-032).
...
- **SC-012**: Launching the package while an instance is already running never starts a second server — it focuses the existing instance (opens the browser to it) and exits, so there is exactly one Alice server and one SQLite connection at a time.
...
- **Robust single-instance coordination is deferred to 041.** 040 ships the minimal port-only single-instance probe (FR-033) with the limitations documented in Edge Cases. 041 MUST replace it with a robust mechanism (per-install lock recording PID + bound port with stale-lock handling, or an equivalent identity-aware handshake) because self-update requires guaranteeing exactly one instance across stop → swap → restart — the old process must fully exit and release the Windows file locks on `runtime\node.exe` / the native `better-sqlite3` binary before program files are replaced, and no two instances may hold `data/alice.db` open during a migration. The mechanism must be **forward-compatible across versions** (an update replaces `server/portable.js`, so a new version's launcher must detect/hand off from an old version's running instance). Recorded in the 041 brief (`docs/features/2.0.0-smart-intake-ai-assistance/041-self-update-support.md`).
