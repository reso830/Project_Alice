# Research — 028 Archive Applications View
...
- `getById` does **not** filter on `archived` — it returns archived rows by id. This is correct: card → overlay → modal flow needs to load archived rows by id for the read-only overlay.
...
- A new query path is required to read **only** archived rows. Two options considered (see §5 below); decision: a query param on the existing list endpoint.

### 1.2 · Supabase (hosted) repo

- `server/repositories/supabase/applications.js:76-85` mirrors SQLite: `.eq('archived', false)` filters the active list.
- `archive()` (`:144-159`) explicitly writes `{ archived: true, fav: false, updated_at: now }` — duplicate of SQLite's behavior.
...
- Schema migration: the Supabase `applications` table gains an `archived_date date` column (nullable, default NULL) via inline SQL pasted in the Supabase SQL editor at deploy time. This project does not use Supabase CLI migration files — canonical pattern from [025-application-timeline/data-model.md § 4.1](../025-application-timeline/data-model.md#L165-L175). Existing rows have NULL → the card date-stamp falls back to `lastStatusUpdate` per the design.

### 1.3 · Shared `toRow()` helper
...
- For `archived_date`, the field is **server-set, never client-supplied**. We deliberately do **not** add it to `updateSchema` — clients should not be able to write it directly. Tests must assert that a PATCH containing `archivedDate` is ignored (or rejected, depending on the strictness mode of `safeParse`).
...
- `api.getAll()` (verify in file) currently has no query param — it always returns the active list.
...
### 2.2 · `src/data/demoStore.js`

- **Critical mismatch.** Today `demoStore.archive(id)` literally **splices the row out of the array** (`:153-158`). There is no archive flag retained in demo mode — once "archived," the row is gone.
...
- The demoSeed (`src/data/demoSeed.js`) currently has no archived rows. Decision: seed two archived demo rows so a fresh demo session has something to show in the Archived view. Otherwise the Profile "Archived applications · 0 →" link leads to an empty state, which is functionally correct but does not demo the feature.

### 2.3 · `src/data/store.js` (localStorage path)

- Legacy localStorage path appears used as a fallback (`STORAGE_KEY = 'apptracker_applications'` at `:8`). Verify whether it is still in the live wire-up (likely deprecated or used only for offline-resilience). If still active, the same archive ↔ unarchive semantics apply.

## 3 · Client UI surface

### 3.1 · Tracker page (`src/pages/Tracker.js`, 522 lines)
...
- Needs:
  ...
  - Wire the new `↺` unarchive handler on archived cards.

### 3.2 · Toolbar component (`src/components/Toolbar.js` — verify file)

- Needs a new `view-chip` element (per `tracker.md` § View switcher) wrapping the title label + count badge in one pill, with a chevron and a click handler that opens the view popup.
- New `view-popup` component (likely inline in Toolbar.js, similar to `QuickFiltersStatusPopup.js`) — backdrop-on-outside-click, header label "View", two option rows (Active + Archived) with dot/label/count-pill grid, indigo highlight on the selected row.
- Count badges inside the popup reflect the **filtered** counts for both views, computed alongside the active-list render.

### 3.3 · Card component (`src/components/Card.js`, 202 lines)
...
- Needs an archived-card branch:
  ...
  - "Archived" stamp chip in Row 1's meta cluster (per `tracker.md` § Archived card variant).
  - Date-stamp slot reads `Archived ${archivedDate || lastStatusUpdate}` instead of `Updated ${lastStatusUpdate}`.
  - Quick-actions row collapses to a single ↺ Unarchive button (no ✎, ⇄, 🔗, ★, ×).
  - The unarchive button calls `api.unarchive(id)`, refreshes the list, fires toast `Unarchived.`.

### 3.4 · Modal component (`src/components/Modal.js`, 1048 lines)

- Currently supports `edit` and `create` modes.
- Needs a third `archived` mode (per `application_overlay.md` §12):
  ...
  - Collapse the header action cluster to ↺ + ✕ only. Hide ★, ⇄, 🗄.
  ...
  - ✕ / Esc / backdrop close immediately, no discard confirmation.
  - `Cmd/Ctrl+S` is a no-op.
  - ↺ button calls `api.unarchive(id)`, closes the modal, fires toast `Unarchived.`.

### 3.5 · Profile page (`src/pages/Profile.js`)

- Existing stat tiles (`createStatChipRow` in `:108-121`) use `computeStats` whose source is `getAll()` — already excludes archived. **No behavioral change to existing tiles** is needed for FR-027.
- New: an `Archived applications · N →` link added to the Applications section (location to be confirmed during implementation review — likely directly under the stat chip row or as a separate sub-section affordance).
- The link's count `N` requires a separate fetch since `getAll()` returns only active rows. Options:
  ...
  - Rejected: a dedicated `GET /api/applications/archived-count` summary endpoint — premature optimization for what is at most a few hundred rows per user.
...
- **No code change is required** to satisfy FR-024…FR-026 — they are already satisfied by virtue of the data flow. The tasks document will include an explicit verification test rather than a code change.

### 3.7 · Styles (`src/styles/main.css`, 6606 lines)

- Today: no CSS for `.view-chip`, `.view-popup`, `.card-archived`, `.card-archived-stamp`, `.archived-stamp`, `.modal-header--light/dark` archived chip variants, the ↺ Unarchive button variants, or the Profile archived link.
...
- `tests/components/Modal.test.js:1360` — asserts `fav: false` after archive. Update to assert `fav` is preserved (test passes both `fav: true` initial state and an unchanged post-archive value).
- `tests/components/Modal.test.js:144, 310, 366, 389` — header action cluster expectations. Verify these still hold for `edit` / `create` modes after the archived mode is added (no regression).
- `tests/components/Card.test.js:93-97` — assert the existing archive button SVG. New test cases cover the archived-card variant having only ↺ and not ×.

### 4.2 · New test files

- `tests/server/routes-protected.test.js` — extend to cover `POST /:id/unarchive` (auth required, 404 when not owner / not found, 200 with record on success).
- New `tests/server/db/archive.test.js` (or co-located with existing applications db tests) — round-trip: archive sets `archived_date`; unarchive clears it; both leave `fav`, `status`, `last_status_update` untouched.
- `tests/data/demoStore.test.js` — round-trip parity assertions: archive keeps the row in the array; getAll excludes archived; getAllArchived returns only archived; unarchive flips back; `fav` preserved across both.
- `tests/services/api.test.js` — `api.unarchive(id)` posts to the right URL; `api.getAll({ view: 'archived' })` sends `?view=archived`.
- `tests/pages/Tracker.test.js` — view chip toggles; URL sync round-trip; pagination resets on switch; filters/sort persist; +New button hidden in archived view; archived card unarchive flow.
- `tests/components/Modal.test.js` — archived mode header chip; no Save/Discard footer; field click is inert; ↺ button calls unarchive and closes the modal; Esc/backdrop close immediately (no discard dialog).
- `tests/pages/Profile.test.js` (or `ProfileEdit.test.js` if profile suite is split) — `Archived applications · N →` link renders with the correct count, including `0`; clicking it navigates to `Tracker.html?view=archived`.
- `tests/pages/Calendar.test.js` — verification test: an archived row's timeline entries do **not** appear in the Calendar's Today / Upcoming / Month Grid / Suggested Actions sections (already true by virtue of data source; lock in with an explicit test).

## 5 · Decisions made at planning

### 5.1 · Archived-list endpoint shape (FR-010)
...
- Matches the URL parameter the client already uses for the view switcher (single source of truth — the server's `?view=archived` and the browser's `?view=archived` mean the same thing).
...
**Rejected alternative:** a dedicated `GET /api/applications/archived` endpoint — duplicates the entire route handler, validation, and RLS wiring for no architectural gain.

### 5.1.1 · Supabase `archive()` / `unarchive()` concurrency hardening
...
.eq('archived', false)         // ← race-killing predicate
```
...
**Rationale:**
- **Eliminates the read-then-write race window.** Two concurrent `archive(id, t1)` and `archive(id, t2)` calls cannot both write `archived_date` — Postgres serializes the UPDATEs, and the second one's `archived = false` predicate matches 0 rows (because the first call set `archived = true`).
...
- **Symmetric across SQLite and Supabase.** Both adapters use the same atomic conditional UPDATE pattern with an `archived = <opposite>` predicate. This eliminates the SQL-level `COALESCE` and ensures FR-033 mode parity — `updated_at` is bumped only on actual transitions (not on idempotent re-calls) in both paths. The earlier draft used SQL-level COALESCE on SQLite, which would have bumped `updated_at` on re-archive while Supabase did not. Symmetric predicates close that gap.
- **Symmetric for unarchive too.** The same pattern applies for unarchive with `.eq('archived', true)` (or `AND archived = 1` in SQLite). Both write paths (archive ↔ unarchive) are race-free and idempotency-clean by construction.
...
### 5.2 · `?view=archived` URL preservation across sign-in redirect
...
### 5.3 · Demo seed includes archived rows

**Decision:** Add two archived rows to `src/data/demoSeed.js` so a fresh demo session has visible archive content.

**Rationale:**
- Without seed data, the demo view's only state is "Nothing archived yet." — which doesn't demonstrate the feature.
...
### 5.4 · `fav` preservation: remove both clearing sites
...
**Rationale:**
- The spec mandates `fav` survives the archive ↔ unarchive round-trip (FR-009).
...
### 5.5 · Calendar exclusion: no code change
...
### 6.1 · `archived_date` migration on Supabase

The hosted Supabase project needs a schema migration to add the `archived_date` column. Risks:

- Existing archived rows have no archived_date. The spec covers this — the card date-stamp falls back to `lastStatusUpdate`. Verify the migration is non-blocking (nullable column, no backfill required).
...
### 6.2 · `fav` behavior change is observable

Removing the `fav = 0` side effect of archive is a **user-observable behavior change** to existing archive. A user who archived a starred row in the past and re-opens the archive view (post-deploy) will not see this change retroactively — `fav` was already cleared in their stored record. The change is forward-only.

- Mitigation: the spec calls this out (FR-009 + Out of scope). Document in CHANGELOG.

### 6.3 · Demo mode semantic change

The demoStore refactor — from "archive splices the row out" to "archive sets a flag" — changes the in-memory shape of `_applications` for any test or component that introspects it directly.
...
### 6.5 · Filter/sort persistence across switch
...
- Mitigation: confirmed by reading Tracker.js — the filter/sort state is page-level. Just don't reset on view switch.

### 6.6 · Concurrent archive / unarchive writes on Supabase *(closed at design time)*
...
- **Resolution:** see [§ 5.1.1](#511--supabase-archive--unarchive-concurrency-hardening). Both adapters use an atomic conditional UPDATE with an `archived = <opposite-state>` predicate. The second (concurrent) call matches 0 rows and falls back to `getById`, returning the first call's record unchanged. Race window eliminated at the SQL layer for SQLite (`AND archived = 0` / `AND archived = 1` clauses) and for Supabase (`.eq('archived', false)` / `.eq('archived', true)` clauses).
- **Symmetry:** the same pattern protects concurrent `unarchive(id)` calls symmetrically — the `archived = true` predicate makes the second concurrent unarchive a no-op. Both write paths (archive and unarchive) are race-free by construction.
- **Test coverage:** [Task 01.4 validation cases #2 and #3](tasks.md#-task-014--sqlite-repo-behavior-fix--new-unarchive--archived-list-query) and [Task 01.5 validation case #4](tasks.md#-task-015--supabase-repo-behavior-fix--new-unarchive--archived-list-query--migration-sql) — assert the adapter-pattern wiring. True race-freedom under genuine concurrent writes is a Postgres / better-sqlite3 serialization guarantee, not a JS-testable property.

## 7 · Validation approach

See [plan.md § Validation](plan.md#validation) for the test pyramid. Quick summary:
