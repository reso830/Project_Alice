...
- **Profile entry point**: Profile page adds an `Archived applications · N →` link that deep-links to `/?view=archived` (per `tracker.md` § Navigation Pages).
...
**Independent Test**: With at least one archived row in the store, load `/`, click the toolbar chip, choose `Archived`. The list shows only `archived = true` rows. The URL contains `?view=archived`. Reload — the Archived view is still active. Click the chip → `Applications` — the URL param clears and the active list returns.
...
**Independent Test**: Seed N archived rows. Load the Profile page. The Archived link shows `N`. Click the link → URL is `/?view=archived` and the Archived view is active.
...
- **FR-028**: The new Profile `Archived applications · N →` link MUST navigate to `/?view=archived` and MUST show the count of archived rows for the current user. The link MUST always be rendered regardless of count — when the user has zero archived rows, it MUST render with `N = 0` (clickable; leads to the Archived empty state).
