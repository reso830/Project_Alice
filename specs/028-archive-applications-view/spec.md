...
- Q: Profile stat tiles (Total / Active / Offers / Rejections) — include archived rows or exclude them? → A: All four tiles **exclude** archived rows. The new "Archived applications · N →" link is the only surface where archived rows are counted. Resolves C2 / FR-027.
- Q: Restore endpoint shape — dedicated route, generic PATCH, or toggle archive endpoint? → A: Dedicated `POST /api/applications/:id/unarchive`, mirroring the existing `archive` endpoint. Client gains an `api.unarchive(id)` helper; demoStore and the Supabase repo each gain an `unarchive()` method. Resolves C4 / FR-007 / FR-010.
- Q: Favorite state across archive ↔ restore — should archive clear `fav`, and should unarchive restore it? → A: **Stop clearing `fav` on archive.** Archive leaves `fav` untouched; `fav` survives the round-trip naturally with no schema additions. The current `SET fav = 0` side effect in `archive()` is removed. Resolves C1 / FR-009 (and is a small intentional behavior change to existing archive).
- Q: Should the search query persist across view switches? → A: **N/A — withdrawn.** Verified the Tracker has no text-search input today; the brief listed "search" in its compatibility list but the feature does not exist in the codebase. C3 is therefore vacuous. Resolves C3. (The draft FR-031 for search persistence was removed; subsequent FRs were renumbered to close the gap. FR-031 in the current spec now refers to persistence, unrelated to C3.)
- Q: Profile "Archived applications · N →" link — hide at count = 0 or always show? → A: **Always show.** Count reads `0` when no rows are archived. Keeps layout stable and makes the surface discoverable before the user has any archived rows. Resolves the inline open question under US-4 Acceptance Scenario 3.

### Withdrawn at this session (no longer applicable)

- C3. Search query persistence — verified no text-search input exists in the Tracker today. The brief's compatibility list mentioned "search" but the feature does not exist. Vacuous; revisit only if/when search is added.
- C5. Imported-from-demo archived rows on hosted sign-in — verified no demo→hosted import flow exists in the codebase today (feature 020 keeps demo state in memory; nothing crosses into hosted on sign-in). Edge case is vacuous; revisit only if/when an import flow is added.

---

## Problem Statement

Applications accumulate. As a user works through a search, many entries reach a state where they no longer warrant attention (closed, lost interest, deferred, ghosted-then-forgotten) but still hold information the user may want to revisit — interview notes, salary data, contact names, the exact job posting URL. Permanently deleting them is too destructive; leaving them in the active list dilutes every count, every filter result, and every suggestion.

Today the codebase has half of an archive lifecycle: the data model carries an `archived` boolean (server-side filtered out of the active list by `WHERE archived = 0`), the Tracker card and Application Overlay expose an Archive button, and `POST /api/applications/:id/archive` works. **But once an application is archived it disappears with no way back** — there is no archived-list view, no unarchive endpoint, no UI surface to restore a row.

This feature closes the loop. It adds a dedicated **Archived view** of the Tracker (toggled via a toolbar chip and deep-linked via `?view=archived`), surfaces an **Archived applications · N →** entry point on the Profile page, gives archived cards and the Application Overlay a **read-only Archived mode**, and introduces a one-click **Unarchive** action that restores a row to the active list. Archived rows remain fully excluded from active workflows, suggestion engine inputs, dashboard counts, and calendar-derived activity unless restored.
...
- Toolbar **View switcher chip** on the Tracker that toggles between Active and Archived lists (per `tracker.md` § Toolbar > View switcher).
...
- **Archived card variant** (per `tracker.md` § Card > Archived card variant): "Archived" stamp chip, date-stamp slot reads "Archived [date]", quick-actions row collapses to a single Unarchive (↺) button. Status accent and badge remain at full strength.
- **Archived Application Overlay mode** (per `application_overlay.md` §12): same frame and header, "ARCHIVED" header chip, ↺ Unarchive + ✕ Close as the only icon actions, all body fields read-only, footer hidden, no draft/dirty machinery, no discard confirmation.
- **Unarchive action**:
  - On card row: clicking ↺ immediately clears `archived` (and the new `archivedDate`); row leaves the Archived list; toast `Unarchived.`. No confirmation.
  - In overlay: clicking ↺ clears `archived` (and `archivedDate`), closes the modal, toast `Unarchived.`. No confirmation.
...
- **Profile entry point**: Profile page adds an `Archived applications · N →` link that deep-links to `Tracker.html?view=archived` (per `tracker.md` § Navigation Pages).
...
- **Exclusion of archived rows** from every active workflow surface:
  ...
  - Suggestion engine inputs (feature 026 Calendar's `followup`, `feedback`, `interview_followup`, `offer_expiry`, `ghost` rules — must not consider archived rows)
  - Calendar Action Panel (`Today`, `Suggested Actions`, `Upcoming`)
  - Calendar Month Grid status chips
  - Profile dashboard counts — Total, Active, Offers, and Rejections all exclude archived rows (per FR-027); only the new "Archived applications · N →" link reports archived counts
...
- A `fav_before_archive` shadow column or any other mechanism to remember a prior favorite state — the spec's chosen rule (FR-009) is that archive does not clear `fav`, so no remembering is needed.
- Restoring previously archived rows via undo toast (the brief and tracker design omit Undo on archive; only the `application_overlay.md` §8 row mentions a toast with Undo — current source has no Undo wiring and adding it is out of scope here).

---

## User Scenarios & Testing

### User Story 1 — Open the Archived view (Priority: P1)

The user is on the Tracker. They click the leading toolbar chip ("Applications ▾"). A popup lists two views — `Applications` and `Archived` — each with a count. They click `Archived`. The list is replaced with their archived applications; pagination resets to page 1; existing filters and sort remain applied. The toolbar chip now reads "Archived ▾" and shows the archived count. The URL gains `?view=archived`. The "+ New application" button and mobile FAB are hidden.
...
**Independent Test**: With at least one archived row in the store, load `/Tracker.html`, click the toolbar chip, choose `Archived`. The list shows only `archived = true` rows. The URL contains `?view=archived`. Reload — the Archived view is still active. Click the chip → `Applications` — the URL param clears and the active list returns.
...
2. **Given** the user is on `/Tracker.html?view=archived` directly (refresh or deep link), **When** the page loads, **Then** the Archived view is the initial state without an intermediate flash of the Active view.
...
### User Story 2 — Unarchive an application from the card (Priority: P1)

The user is in the Archived view. Each card shows the existing status accent and badge plus a small "Archived" stamp chip, a date-stamp that reads "Archived [date]", and a single ↺ button in the actions row. They click ↺ on one card. The row immediately disappears from the Archived list, the toolbar chip's archived count drops by one, and a toast reads `Unarchived.`. No confirmation dialog is shown.

**Why this priority**: This is the primary mechanic that gives the archived view its purpose — recovery without ceremony.

**Independent Test**: With one archived row in the store, switch to the Archived view, click ↺. The row leaves the Archived list, the toast fires, and the row is now present in the Active view with `archived = false` and `archivedDate = null`. Status is unchanged.

**Acceptance Scenarios**:

1. **Given** an archived row with status `rejected`, **When** the user clicks ↺, **Then** the persisted record has `archived = false`, `archivedDate = null`, and `status = rejected` (unchanged), and the row appears in the Active list.
2. **Given** the user is on the Archived view and clicks ↺, **Then** no confirmation dialog is shown (contrast with archive, which does confirm).
...
### User Story 3 — Open an archived application in read-only overlay (Priority: P2)

The user clicks anywhere on an archived card body (or its ↺ does not catch the click). The Application Overlay opens in **Archived mode**: same header layout, but a small "ARCHIVED" chip sits next to the status badge, the only header actions are ↺ and ✕, all body fields render their values without entering edit mode, and the Save/Discard footer is hidden entirely.
...
**Independent Test**: With one archived row containing populated responsibilities, skills, salary, URL, and notes, click the card body. The overlay opens. Click any body field (e.g. Company) — nothing happens. Press Esc — overlay closes immediately without a discard confirmation.

**Acceptance Scenarios**:

1. **Given** the user clicks an archived card, **When** the overlay opens, **Then** the mode is `archived`, an "ARCHIVED" chip is rendered in the header, only ↺ and ✕ are present in the header action cluster (no ★, ⇄, or 🗄), and the footer is not rendered.
...
3. **Given** the overlay is in Archived mode, **When** the user presses Esc / clicks the backdrop / clicks ✕, **Then** the overlay closes immediately with no discard confirmation.
4. **Given** the overlay is in Archived mode, **When** the user clicks the status badge, **Then** the Status Dropdown does **not** open.
5. **Given** the overlay is in Archived mode, **When** the user clicks ↺ in the header, **Then** the row is unarchived, the modal closes, and the toast `Unarchived.` fires.

---

### User Story 4 — Profile entry point to the Archived view (Priority: P2)

The user opens the Profile page. Alongside the existing stat tiles (Total, Active, Pending, Offer — per the actual implementation in [src/pages/Profile.js:113-118](../../src/pages/Profile.js#L113-L118); note this differs from the design doc's "Total / Active / Offers / Rejections" label set, which describes a redesign that has not yet shipped) they see an `Archived applications · N →` link showing the count of archived rows. Clicking it navigates to `Tracker.html?view=archived`.

**Why this priority**: Without a Profile entry point, the only way to reach the Archived view is from the toolbar chip after already loading the Tracker — discoverable but indirect. The Profile link is the secondary discovery surface and is explicitly called out in the design.

**Independent Test**: Seed N archived rows. Load the Profile page. The Archived link shows `N`. Click the link → URL is `Tracker.html?view=archived` and the Archived view is active.

**Acceptance Scenarios**:

1. **Given** 3 archived rows exist, **When** the user opens the Profile page, **Then** the Archived link reads `Archived applications · 3 →`.
2. **Given** the user clicks the Archived link, **Then** the browser navigates to `Tracker.html?view=archived` and the Archived view is the initial state.
...
### User Story 5 — Archive a row and have it leave active surfaces (Priority: P2)
...
**Independent Test**: Seed one application that would otherwise trigger each of the Calendar suggestion rules (`followup`, `feedback`, `interview_followup`, `offer_expiry`, `ghost`). Archive each one. Open the Calendar — the Suggested Actions panel is empty for those rows; the Action Panel's Today / Upcoming sections do not list them; the Month Grid does not draw chips for their timeline entries.
...
### User Story 6 — Filter, sort, paginate within the Archived view (Priority: P3)
...
**Independent Test**: With 25 archived rows across mixed statuses and companies, switch to Archived, apply a `Status = Rejected` filter — only matching archived rows are shown; pagination chunks correctly. Change sort to `Salary desc` — order updates. Switch back to Active — the same filter/sort is still applied and applies to active rows.
...
- **Restore of a row with pending follow-up data**: any follow-up fields, Timeline entries, notes, and skills survive the archive ↔ unarchive round-trip unchanged.
- **Large archived datasets**: filter/sort/pagination apply to the Archived list with the same semantics and limits as the Active list. The Archived count badge reflects the **filtered** count, matching the Active chip's existing behavior. (The brief's mention of "search" in its compatibility list does not apply — no Tracker text-search input exists today.)
...
- **Hosted mode import** of demo data on first sign-in: no such flow exists today (verified at clarify-time; see Clarifications § Withdrawn C5). Revisit only if a demo→hosted import flow is introduced.
- **Race conditions**: if the user clicks ↺ twice in rapid succession (or ×/Archive twice on the same row), the second request returns 200 with the same already-transitioned record per FR-008 idempotency. The card is gone from the visible list after the first response, so the second click would typically come from the modal's archive button before its first response landed. UI should not surface a misleading error.
- **Archiving while in the Archived view**: not reachable — archived cards' quick-actions row collapses to a single ↺ Unarchive button per FR-013 (no × Archive button), and the archived-mode overlay hides the 🗄 Archive icon per FR-017. The "+ New application" toolbar button and FAB hide (FR-004) prevent creation, which is a separate concern.
- **URL with `?view=archived` while signed out**: the route is gated like the rest of the Tracker — unauthenticated visitors are routed to the Welcome page (see `docs/design/welcome_page.md`). Whether the `?view=archived` parameter is preserved across the sign-in redirect (so the user lands on the Archived view after signing in) or dropped (lands on the default Active view) is deferred to planning; either behavior satisfies the spec.
...
- **Profile archived-count fetch failure**: the Profile page issues a separate `api.getAll({ view: 'archived' })` call to compute the count for the `Archived applications · N →` link. On fetch failure (network error, server 5xx, etc.) the link renders with `N = 0` as a graceful-degradation default rather than blocking the entire Profile render. The user can still click through; the Archived view itself surfaces the load/error state per FR-036. This is an explicit UX choice: an inaccurate "0" is preferable to a broken Profile page, since the Archived view's own error surface is the canonical place to report list-fetch failures.
- **Star (`fav`) preservation across archive ↔ unarchive**: `fav` is untouched by archive and by unarchive. The current archive behavior of zeroing `fav` is being removed (see FR-009). A row archived while favorited remains favorited (the ★ icon is just hidden in the archived card/overlay surfaces) and re-emerges with `fav = true` on unarchive.
...
- **FR-008**: Archive and unarchive MUST be idempotent — archiving an already-archived row or unarchiving an already-active row MUST be a safe no-op (response shape consistent with other writes).
- **FR-009**: Unarchive MUST NOT change `status`, `fav`, `lastStatusUpdate`, `applicationDate`, or any other field beyond `archived` and `archivedDate`. It is a pure restore. Symmetrically, archive MUST also leave `fav` untouched — the current behavior of zeroing `fav` on archive is removed so that `fav` survives the archive ↔ unarchive round-trip naturally.
...
- **FR-013**: Archived cards MUST use the layout described in `tracker.md` § Archived card variant: identical surface/border/shadow/status accent to active cards, with an "Archived" stamp chip in Row 1, a date-stamp reading "Archived [archivedDate || lastStatusUpdate fallback]", and a quick-actions row collapsed to a single ↺ Unarchive button (no ✎, ⇄, 🔗, ★, ×).
- **FR-014**: Clicking the card body of an archived card MUST open the Application Overlay in Archived mode (FR-018). Clicking the ↺ button MUST NOT open the overlay; it MUST execute the unarchive operation.

#### Application Overlay — Archived mode

- **FR-015**: The Application Overlay MUST support a third mode, `archived`, in addition to `edit` and `create` (per `application_overlay.md` §12).
- **FR-016**: In Archived mode the header MUST render the existing ID pill and status badge plus an "ARCHIVED" chip (`archived-stamp`) with the contrast-aware styling defined in `application_overlay.md` §12.1. The status badge and the job title MUST NOT be interactive.
- **FR-017**: In Archived mode the header action cluster MUST collapse to exactly two buttons: ↺ Unarchive and ✕ Close. The ★ Favorite, ⇄ Change Status, and 🗄 Archive icons MUST be hidden.
...
- **FR-020**: In Archived mode the Status Dropdown MUST be unreachable (no badge click handler, no ⇄ icon).
- **FR-021**: In Archived mode ✕ / backdrop click / Esc MUST close the modal immediately, never showing a discard confirmation. `Cmd/Ctrl + S` MUST be a no-op.
- **FR-022**: Clicking ↺ Unarchive in the overlay MUST perform the unarchive operation, close the modal, and fire toast `Unarchived.`. No confirmation dialog.
...
- **FR-027**: The existing Profile stat tiles (Total, Active, Pending, Offer — per the actual implementation; the design doc shows a different label set ("Total / Active / Offers / Rejections") that describes a redesign not yet shipped — see [src/pages/Profile.js:113-118](../../src/pages/Profile.js#L113-L118) and [src/models/profile.js:420-429](../../src/models/profile.js#L420-L429)) MUST exclude archived rows from their counts. Each tile counts only rows with `archived === false`. Today this is satisfied by-default because the Profile reads from `getAll()` which already excludes archived; no Profile code change is required to honor this FR. The "Archived applications · N →" link (FR-028) is the sole Profile surface that reports archived rows.
- **FR-028**: The new Profile `Archived applications · N →` link MUST navigate to `Tracker.html?view=archived` and MUST show the count of archived rows for the current user. The link MUST always be rendered regardless of count — when the user has zero archived rows, it MUST render with `N = 0` (clickable; leads to the Archived empty state).
...
- **FR-038**: System MUST preserve required job application fields: company name, job title, status, last_status_update, responsibilities (Constitution Amendment 1.2.0) — archive and unarchive do not alter these.
...
- **FR-041**: System MUST provide automated tests for the new logic: archive sets `archivedDate`; unarchive clears it; `status` and `fav` preservation across the archive ↔ unarchive round-trip (per FR-009); URL `?view=archived` parsing; Calendar exclusion rules.

---

### Key Entities

- **Application** *(existing — extended)*: the tracked job application record. New behavior:
  - `archived: boolean` — already exists; semantics unchanged.
  - `archivedDate: ISO date string | null` — **new**. Set automatically by the server when `archived` transitions `false → true`. Cleared automatically when it transitions `true → false`. Surfaced on the card date-stamp and in any future audit context.
...
- **SC-001**: A user with archived applications can reach the Archived view from a cold load in at most two interactions: either via the toolbar chip (1 click → 1 select) or via the Profile link (1 click).
...
- **SC-006**: Zero regressions to existing archive behavior — the current `× Archive` button on active cards and the 🗄 Archive in the Application Overlay continue to behave as before (confirm dialog, toast, row leaves the Active list).
...
- Existing tracker filters, sort, and pagination components can be reused without forks for the Archived view. (No Tracker text-search input exists today — C3 withdrawn at `/speckit.clarify`.)
...
- The `archivedDate` field can be added without a data migration that disrupts running deployments — existing archived rows will simply have `null` and fall back to `lastStatusUpdate` on display.
- The Profile page (feature 007) can accept a new link tile without a redesign — only an additive change to the stat tile row.
