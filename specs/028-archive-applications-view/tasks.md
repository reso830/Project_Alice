...
- "Archived applications · N →" link on the Profile page (deep-linked to `/?view=archived`).
...
- With at least one archived row, load `/`. Click the toolbar chip → select `Archived`.
...
- ✅ Clicking the link navigates to `/?view=archived` and the Archived view is the initial state (no flash).
