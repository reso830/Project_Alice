...
|---|---|---|---|
| 01 | Setup (Assets Staging & Housekeeping) | T001–T002a | Initialize folder structure, stage handoff files, and wire favicons |
...
- [x] T001 Initialize logo and graphics directories under `src/assets/`
...
- [x] T002 Copy brand marks, empty-states, and favicons from `design_handoffs/`
...
- [x] T002a Wire the favicon set into `index.html`'s `<head>`
...
- [x] T003 Integrate standardized line-icon registry in `src/utils/icons.js`
...
- [x] T004 Validate line-icon registry shim exports
  - **Target**: [src/utils/icons.js](../../src/utils/icons.js)
...
- [x] T005 [P] [US1] Update global navigation bar logo
...
- [x] T006 [P] [US1] Update global footer brand logo
...
- [x] T007 [P] [US1] Update Config Error page logo
...
- [x] T008 [US1] Update welcome page layout viewports in `WelcomePage.js`
...
- [x] T009 [P] [US1] Integrate full-color vector logo into welcome page headers
  - **Target**: [src/pages/welcome/WelcomePage.js](../../src/pages/welcome/WelcomePage.js)
...
- [x] T010 [P] [US1] Refactor Welcome Page CSS breakpoints and layouts
...
- [x] T011 [US1] Build mini footer with version and repository details
  - **Target**: [src/pages/welcome/WelcomePage.js](../../src/pages/welcome/WelcomePage.js)
...
- [x] T012 [P] [US2] Update QuickFiltersToolbar to use registry icons
...
- [x] T013 [P] [US2] Update Card and Modal to use registry icons
...
- [x] T014 [P] [US2] Update ProfileEdit form to use registry icons
...
- [x] T015 [P] [US2] Integrate ActionPanel empty states illustration
...
- [x] T016 [P] [US2] Integrate DayPanel compact empty states illustration
...
- [x] T018 [US3] Reconcile and update showcase scene modules under src/pages/welcome/scenes/
...
- [x] T019 [P] [US3] Update brand white sigil path inside SceneDeck.js
...
- [x] T020 [US3] Implement prefers-reduced-motion accessibility rules
...
- [x] T021 [US4] Redesign Auth Overlay modal shell and forms
...
- [x] T022 [US4] Rebuild Resume Import in-app LLM processing overlay
...
- [x] T023 [US4] Rebuild Job Posting Import in-app LLM processing overlay
...
- [x] T024 [P] [US4] Disable in-app loader animations for reduced motion
...
> **Phase 06 traceability (impl. notes):**
> - Loader/auth CSS added to `src/styles/main.css` (`.processing-overlay`, `.spinner-ring`/`--wrap`/`--logo`, `.edge-glow`, `.auth-form__input-wrap`/`__password-toggle`/`__field--error`, `.auth-overlay__subtitle`/`__header-copy`). `main.css` is a feature-level Affected Area (also used by T010), not a T021–T024 target — noted for scope traceability.
> - Edge-glow rotation gated by `@media (min-width: 900px)` per spec (not the `pointer: fine` proxy); static box-shadow bloom below 900px. Reduced-motion freezes spinner + edge-glow and drops the fade.
> - 400ms ease-out fade-out (T022/T023): overlay detaches to `<body>` and gets `.processing-overlay--closing` before removal; unit-covered for presence/copy, fade visually verified in the T035 browser smoke test.
> - **Bug fixed during T021**: `SignupForm.js` `field()` had a stray `wrap.append(label, input, fieldError)` before `replaceChildren`, orphaning the signup inputs (they never entered the DOM). Removed; the existing auth unit tests now pass unchanged.
...
- [x] T025 Update affected Vitest test files to reflect brand asset path changes, scene file renames, and version bump sync
  - **Target**: 
...
- [x] T026 Audit HTML5 semantic layout structure & accessibility tags
...
> **Phase 07 implementation notes:** `tests/build/favicon.test.js` now covers all four favicon head links and verifies their staged `public/` files exist. Version-bump assertions in `tests/release-metadata.test.js`, `tests/pages/Profile.account.test.js`, and `tests/pages/profile.aiSettings.test.js` remain at `1.10.8` until Phase 08 performs T027-T031. Pulling `1.11.0` assertions into Phase 07 would make the suite red while the authoritative version surfaces are still intentionally unchanged.
...
- [x] T027 Bump version across all project surfaces (from 1.10.8 to 1.11.0)
...
- [x] T028 Update Project Alice feature roadmap status
...
- [x] T029 Add release entries to CHANGELOG
...
- [x] T030 Update repository file map references
...
- [x] T031 Complete documentation sanity checks
