...
function withApplicationsRepository(test) {
  const db = makeMemoryDb();
  const repositories = createTestRepositories(db);

  try {
    test(repositories.applications);
  } finally {
    db.close();
  }
}
...
describe('SQLite applications repository', () => {
  it('returns an empty list for a fresh database', () => {
    withApplicationsRepository((repo) => {
      expect(repo.getAll()).toEqual([]);
    });
  });

  it('creates and reads an application record', () => {
    withApplicationsRepository((repo) => {
  ...
  it('updates an application record and returns null for an unknown id', () => {
    withApplicationsRepository((repo) => {
      const record = repo.create(validApplication());
  ...
  it('archives an application record and returns null for an unknown id', () => {
    withApplicationsRepository((repo) => {
