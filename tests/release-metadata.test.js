...
describe('release metadata', () => {
  it('keeps the 1.10.4 release version in sync across package, app chrome, and docs', () => {
    expect(pkg.version).toBe('1.10.4');
    expect(APP_VERSION).toBe('v1.10.4');

    const lock = JSON.parse(read('package-lock.json'));
    expect(lock.version).toBe('1.10.4');
    expect(lock.packages['']?.version).toBe('1.10.4');

    expect(read('README.md')).toContain('Current version: **1.10.4**');
    ...
    expect(read('CHANGELOG.md')).toContain('[Unreleased]: https://github.com/reso830/Project_Alice/compare/v1.10.4...HEAD');
