...
- **Faster hosted startup** — a branded loader paints instantly on hosted cold loads instead of a blank white page, the boot handshake runs the health check and session check in parallel instead of sequentially so signed-in/signed-out visitors don't wait on the serverless cold start, `Calendar`/`Profile`/`ProfileEdit` code-split into separate chunks loaded on first visit (`Tracker` stays eager), and fonts are self-hosted instead of a third-party Google Fonts request. Local, portable, and demo boot are unchanged. See [`specs/044-hosted-startup-performance/`](specs/044-hosted-startup-performance/). (044-hosted-startup-performance)
...
Current version: **1.12.11** — see [CHANGELOG.md](CHANGELOG.md)
