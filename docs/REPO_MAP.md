...
| `scripts/start-alice.mjs` | Cross-platform clone launcher (feature 041, Increment 2) — `npm start`. Supervises the server in a `for(;;)` loop: spawns a `--serve` child (builds `dist/` if missing, serves it via `createApp`), and on a clean child exit with `data/update-pending.json` present, applies the staged **git** update (stash if dirty → `git checkout <release-tag>` → `npm install` → `npm run build`), rolling back to the previous ref + writing `data/update-failed.json` on failure, then relaunches (browser-open suppressed after the first launch). Sets `ALICE_UPDATE_CHANNEL=git` (disabled when not a git repo). Exports `createLauncherEnv` / `resolveGitUpdateCapability` / `applyPendingGitUpdate` / `runServer` / `run` |
