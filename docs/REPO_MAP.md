...
| `server/portable.js` | Portable/local launcher entry (feature 040) — exported `run({ root, open, probe })`; resolves the package root, then **single-instance probe**: fetches the configured port's `/api/health` and, if Alice already answers, opens the browser to it and exits without starting a second server (FR-033). Otherwise sets `APP_RUNTIME=local` + `ALICE_DB_PATH`, **dynamically** imports `config.js`/`repositories`/`index.js` (so they evaluate after env is set), builds the app with `serveStatic:true`, binds `127.0.0.1` via `listenWithFallback`, opens the browser (injectable), logs to `logs/alice.log`, and stops cleanly on `SIGINT` |
