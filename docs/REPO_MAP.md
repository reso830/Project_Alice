...
| `src/services/llmClient.js` | Shared OpenRouter HTTP transport (037) — `requestChatCompletion({ text, key, model, systemPrompt })`, `LLM_TIMEOUT_MS` (30s), `MAX_INPUT_CHARS`, `OPENROUTER_URL`, `createLlmError`, `mapErrorToReason`. Extracted from `llmParser.js`; imported by both `llmParser.js` and `compatNotesService.js` |
| `src/services/llmParser.js` | Browser-direct OpenRouter client — `parseWithLlm(text, key, model)` (resume → profile) and `parseJobWithLlm(text, key, model)` (feature 035; job posting → application draft validated through the application model), both → `{ draft, truncated }` over one shared transport; `validateKey(key)`, `DEFAULT_MODEL` / `LLM_TIMEOUT_MS` (30s) / `MAX_INPUT_CHARS` constants, `REASON_CODES`, and `mapErrorToReason`. Feature 037: HTTP transport moved to `llmClient.js`; all exports unchanged |
| `src/services/compatNotesService.js` | Compatibility notes generation (037) — `generateNotes(application, profile, aiSettings)` builds a system + user prompt (score, tier, resolved skill matches, JD fields, profile fields), calls `llmClient.requestChatCompletion`, parses the JSON response `{ summary, body }`, truncates `summary` to 34 chars if needed |
...
| `tests/services/` | API client, supabase client, resume API, health API, LLM parser (`llmParser.test.js`, 033), compat notes service (`compatNotesService.test.js`, 037) |
