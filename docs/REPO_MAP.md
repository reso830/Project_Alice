...
| `server/health.js` | `assertHostedSchema(config, { logger? })` — hosted-mode boot check; PostgREST sentinel probes against `applications` (including `applications.timeline`), `profile`, `profile_skill` (032), and `user_seed_state`; throws on `42703` / `42P01` (migration not applied); soft-warns on transient errors |
...
| `server/db.js` | SQLite connection and schema creation; `ensureColumn` cluster handles in-place column adds for evolving columns (e.g. `archived`, `timeline`, `archived_date`). Feature 032: creates the `profile_skill` table (one row per profile skill) with a per-profile index and a case-insensitive unique index on `(profile_id, lower(skill_name))` |
...
| `src/services/llmParser.js` | Browser-direct OpenRouter client (feature 033) — `parseWithLlm(text, key)` → `{ draft, truncated }`; `DEFAULT_MODEL` / `LLM_TIMEOUT_MS` (30s) / `MAX_INPUT_CHARS` constants; `AbortController` timeout; sanitizes the assistant JSON via `normaliseProfile`; throws typed errors (caller maps to friendly text + rule-based fallback). The user's key is sent only to OpenRouter, never to Alice's server |
| `src/data/aiSettings.js` | Browser-only AI settings store (feature 033) — `localStorage`-backed `getKey/setKey/clearKey/hasKey` (`alice.ai.openrouterKey`) and `getConsent/setConsent/clearConsent/hasConsent` (`alice.ai.consent`); pure logic, no DOM; the key never leaves the browser except to OpenRouter |
