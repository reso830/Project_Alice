...
| `src/pages/Profile.js` | User profile screen — refreshed overview layout, Archived Applications entry point, Skills meters, and unified Settings card for browser-local AI Settings plus Account controls. Feature 041: when `health.updateSupported`, renders the middle **Updates** Settings sub-group — current version, **Check now**, Connection Error / Update Failed (+ Retry) states, auto-check toggle, and a collapsible update-mode radio picker — hydrating from and polling `/api/update/status` and persisting prefs via `/api/update/settings`. Issue #80: the empty-profile **Set Up Profile** CTA dynamically imports `ProfileEdit.js` and opens its `openSetupGate` Smart/Manual entry gate directly on this page, navigating to Edit Profile only after the gate resolves |
| `src/pages/ProfileEdit.js` | Profile editor — sticky Save/Cancel, dirty-state tracking, section overlays, existing-profile Import Bar, smart paste/upload import, provenance tags, Undo, and `epfFlash` import highlighting. In demo (feature 020) the resume-import slot renders an inline `.profile-edit__resume-demo-note` ("Resume import is available after signing in.") instead of mounting `ResumeImport`. Issue #80: exports `openSetupGate({ navigate, onChooseManual, onImportSuccess, onDismiss })`, the Smart/Manual entry gate rebuilt on the shared `.creation-picker-card` styling (same markup as the Add-application gate); `Profile.js` opens it pre-navigation, and `mount()` still opens it locally as a fallback when landing directly on this page with no profile |
...
| `src/pages/welcome/WelcomePage.js` | Welcome landing page — applies fixed production layout/theme/copy classes plus viewport matchMedia; mounts the hero slideshow on desktop/tablet and tears it down on mobile; `?auth=callback` verification banner handler. Issue #139: hosted-mode-only, non-dismissible limitations banner (`renderLimitationsBanner()`) — `position: fixed`, zero layout footprint on `.welcome`'s own box; `shouldMarquee()` (exported) measures real text overflow to switch to a pause-then-scroll marquee, respecting `prefers-reduced-motion` |
...
| `src/pages/welcome/AuthOverlay.js` | Centered-modal overlay — header (40px Alice mark + title + close), focus trap, ESC/backdrop dismissal, footer with "or" divider + demo button + swap-mode link + signup-only legal copy; `verification_sent` state. Feature 045: adds `forgot`/`forgot_sent` (renders `ForgotPasswordForm.js`, non-enumerating confirmation) and `reset-password`/`recovery-expired` (renders `ResetPasswordForm.js`; only ever opened via `main.js`'s initial-view threading of a confirmed recovery session, never a click). `reset-password`'s close (×/Escape/backdrop/"Back to sign in") is the one view whose abandon path doesn't fall through to the generic close — it also ends the recovery session (`authStore.signOut()`), gated on no submit being in flight, and waits for that sign-out attempt to settle before finishing the close so the overlay can't disappear while the session is still live |
| `src/pages/welcome/LoginForm.js` | Email/password login — neutral error, accessible inline loading. Feature 045: "Forgot password?" link opens `AuthOverlay`'s `forgot` view |
| `src/pages/welcome/SignupForm.js` | Email/password signup — inline validation, neutral rejection, → `verification_sent` |
| `src/pages/welcome/ForgotPasswordForm.js` | Forgot Password request form (feature 045) — single email field, calls `supabase.auth.resetPasswordForEmail(email, { redirectTo: emailRedirectUrl })` for every syntactically-valid address and always proceeds to `AuthOverlay`'s `forgot_sent` confirmation regardless of Supabase's actual response (non-enumeration, FR-8/AC-5). Reuses the existing `VITE_AUTH_EMAIL_REDIRECT_URL` (no second env var) — `WelcomePage.js`'s `?auth=callback` verification banner now also checks `authStore.RECOVERY_URL_MARKER` so a recovery link sharing that redirect URL doesn't trigger the wrong banner |
| `src/pages/welcome/ResetPasswordForm.js` | Reset Password form (feature 045) — new/confirm password fields (independent show/hide toggles, `validatePassword`), mounted only inside `AuthOverlay`'s `reset-password` view. On `supabase.auth.updateUser({ password })` success: stages a success notice then explicitly `authStore.signOut()`s (spec Clarification — a successful reset ends the session rather than showing an in-overlay success card); a `finally` always releases its own pending state even if that `signOut()` rejects, so the form/overlay can't get stuck disabled. On a failure citing an invalid/expired session: transitions `AuthOverlay` to `recovery-expired` instead of a generic error |
| `src/pages/welcome/shared/appMeta.js` | `APP_VERSION` (derived from `package.json` at build/test time via Vite's `__APP_VERSION__` define, issue #132) / `ISSUE_URL` / `LICENSE_NAME` / `LICENSE_URL` — single source consumed by both `Footer.js` and the welcome mini footer. Frontend-only — server code uses `server/appVersion.js` instead, since the Vite define doesn't exist in plain Node |
...
| `index.html` | Vite entry HTML. Issue #139: description, canonical link, Open Graph/Twitter Card tags pointing at `public/og-image.png`; app root is `<main id="app">` (a real landmark, though it's replaced by a fresh `<main>` element from `src/main.js` before any real page content mounts — see that entry) |
| `public/robots.txt` | Issue #139 — disallows only `/api/`; the SPA's client-side router never changes the URL path, so there are no other real frontend paths to gate |
| `public/og-image.png` | Issue #139 — static, pre-rendered 1200×630 Open Graph/Twitter Card social image (Alice sigil + wordmark + tagline on the navy theme). Generated by `scripts/generate-og-image.mjs`, not per-request, matching this project's no-SSR architecture — rerun that script if brand assets or copy change |
| `scripts/generate-og-image.mjs` | Issue #139 — one-off generator for `public/og-image.png` via `@napi-rs/canvas` (already a dependency); run manually with `node scripts/generate-og-image.mjs` |
...
| `src/components/PasswordChangeModal.js` | Change Password modal (feature 045, Profile › Settings › Account, hosted only) — modeled structurally on `DeleteAccountModal.js` (backdrop + dialog + focus trap + ESC/backdrop-close disabled while loading); three fields (current/new/confirm, each with its own show/hide toggle + `aria-live` error region), in-modal success card ("Done" closes, no navigation — the user is already signed in). `onConfirm({ currentPassword, newPassword })`: rejecting `INVALID_PASSWORD` keeps it open with an inline error under Current password, any other rejection closes it and the caller toasts |
...
| `server/appVersion.js` | `APP_VERSION` (issue #132) — reads `package.json` via `createRequire` (plain Node, no Vite). Server-side counterpart to `src/pages/welcome/shared/appMeta.js`'s Vite-`define`-based `APP_VERSION`; consumed by `server/health.js`, `server/portable/lock.js`, `server/routes/update.js` |
...
| `server/routes/account.js` | `DELETE /api/account` (feature 030) — `{ repos, requireAuth? }`; calls the uniform `req.repos.account.delete(body)` and maps typed adapter errors (`VALIDATION_ERROR` 400 / `INVALID_PASSWORD` 401) to responses. Intentionally mounts **no** `seedHostedUserIfNeeded` (no re-seed on the delete path). Feature 045: `PATCH /password` — same repos-dispatched, error-mapping shape, calling `req.repos.account.changePassword(body)`; `currentPassword`/`newPassword` are never logged |
...
| `server/repositories/supabase/account.js` | `createSupabaseAccountRepository({ userId, email })` (feature 030) — hosted `delete(body)`: re-verifies `body.password` via a throwaway anon `signInWithPassword`, then service-role `admin.deleteUser(userId)` (cascade clears the user's rows). Lazy-imports `@supabase/supabase-js` + `adminClient.js` inside `delete()`; password never logged. Feature 045: `changePassword(body)` — validates `currentPassword`/`newPassword` (min length via `PASSWORD_MIN`), re-verifies `currentPassword` the same way `delete()` re-verifies (throwaway anon `signInWithPassword`, distinct `INVALID_PASSWORD` 401 code), then updates via service-role `admin.auth.admin.updateUserById(userId, { password })` |
...
| `server/repositories/account.js` | `createSqliteAccountRepository(db)` (feature 030) — local `delete(body)`: requires `body.confirm === 'DELETE'` (else `VALIDATION_ERROR`), then clears all `applications` + `profile` rows in a single transaction. No re-seed. Feature 045: `changePassword()` unconditionally throws `NOT_SUPPORTED` (501) — local mode has no Supabase-managed password to change |
...
| `src/utils/vercelObservability.js` | `reportVercelObservability({ runtime })`, `reportPageview(page)` (issue #132) — the only allowed import site for `@vercel/analytics`/`@vercel/speed-insights` (enforced by `tests/build/vercel-observability-callsites.test.js`). Gates injection on `runtime === 'hosted'`, suppresses events for the lifetime of any Demo Mode session via a shared `beforeSend`, and redacts Supabase auth-callback URL artifacts before anything is reported |
...
| `tests/utils/vercelObservability.test.js` | `reportVercelObservability`/`reportPageview` (issue #132) — hosted-only gate, `beforeSend` Demo Mode suppression, auth-callback URL redaction, pageview path mapping |
...
| `tests/build/` | Vite build-time env-var assertion (`vite-config.test.js`, incl. `stripStartupLoaderInDev`); favicon asset staging (`favicon.test.js`); feature 044 — `code-splitting.test.js` (real `vite build` asserts `Calendar`/`Profile`/`ProfileEdit` split into separate chunks, `Tracker` stays in the main chunk) and `font-loading.test.js` (built `index.html` has no `fonts.googleapis.com`, self-hosted `@font-face`/`.woff2` present); issue #93 — `notFound.test.js` (asserts `public/404.html` exists, is branded, and self-hosts the Sora 800 weight it needs); issue #132 — `vercel-observability-callsites.test.js` (asserts `@vercel/analytics`/`@vercel/speed-insights` are only ever imported from `src/utils/vercelObservability.js`); issue #139 — `seo.test.js` (description/canonical/OG/Twitter meta, `public/og-image.png`, `public/robots.txt`), `securityHeaders.test.js` (`vercel.json`'s CSP and related headers), `a11y.test.js` (`index.html`'s `<main id="app">` landmark) |
...
| `specs/045-auth-password-reset/` | Hosted Password Management spec package — Change Password (`PATCH /api/account/password`, `PasswordChangeModal`), Forgot Password (`ForgotPasswordForm`, non-enumerating `resetPasswordForEmail` request), Reset Password via a real recovery link (`ResetPasswordForm`, `authStore`'s `password-recovery`/`recovery-expired` states, explicit sign-out on both success and abandon), the shared `validatePassword` policy, and v1.14.0 release prep |
